...
<html lang="en">
  <head>
    ...
    <style>
      ...
      pre { text-align: left; background: #fee2e2; color: #991b1b; padding: 1rem; border-radius: 0.375rem; overflow-x: auto; font-family: monospace; font-size: 13px; border: 1px solid #fca5a5; max-height: 300px; }
    </style>
  </head>
  <body>
    <div class="card">
      ...
      <div style="margin-top: 2rem;">
        <h3 style="text-align: left; font-size: 14px; margin-bottom: 0.5rem;">Diagnostic Error Details:</h3>
        <pre>${error ? error instanceof Error ? `${error.name}: ${error.message}\n\nStack:\n${error.stack}` : typeof error === "object" ? JSON.stringify(error, null, 2) : String(error) : "No detailed error captured."}</pre>
      </div>
    </div>
  </body>
</html>`;
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-CKtwLiJK.mjs").then((m) => m.default ?? m);
